(function(){
	...
	exports.blogs = [
...

		{
			...
			getTitle: function (html) {
				var deferred = $q.defer();
				var title = cheerio.load(html)('.brook-cover h1');
				deferred.resolve(title.text());
				return deferred.promise;
			},
			getHtmlContent: function (html) {
				...
				deferred.resolve(content('.post__content').html());
				return deferred.promise;
			},
			getPostDate: function (html) {
				var deferred = $q.defer();
				var date = cheerio.load(html, cheerioOptions);
				date = date('.meta--date').text();

				var dateParts = date.trim().replace(",", "").split(" ");
				var months = ["jaanuar", "veebruar", "märts", "aprill", "mai", "juuni", "juuli", "august", "september", "oktoober", "november", "detsember"];
				var monthNumber = months.indexOf(dateParts[0]);
				var dateObj = new Date(dateParts[2], monthNumber, dateParts[1], 12);
				console.log(dateObj);
				deferred.resolve(dateObj);
				return deferred.promise;
			},
